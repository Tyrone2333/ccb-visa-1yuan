...
// 需要监控的商品名称列表
let targetStr = ($.isNode() ? process.env['ccb_visa_target'] : $.getdata('ccb_visa_target'))
const targetItems = targetStr ? targetStr.split(',') :  ["20元京东E卡", "信用卡微信立减金券包"]

console.log(targetItems, typeof targetItems)
...
async function checkGoodsList() {
  ...
  try {
    ...
    if (notifyMessages.length > 0) {
      let title = '全球支付 Visa 卡一元购';
      let contents = notifyMessages.join('\n');

      console.log('title:', title, 'contents:', contents);
      sendNotify.sendNotify(title, contents);
